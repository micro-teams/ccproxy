#!/bin/sh
# install.sh — one-line installer for the `ccproxy-connector`, served by the backend.
#
#   curl -fsSL <origin>/ccproxy/install.sh | sh
#
# Downloads the right prebuilt `ccproxy-connector` binary — and a private, static tmux — from
...
# server URL. Enrolment happens separately, with a device token issued when the machine
# was created, so piping this to a shell never grants access by itself.
#
# Recommended: run as a NORMAL user — `curl … | sh`, no sudo. The binary goes in your
# ~/.local/bin and config/tmux in your home, and the next step, `ccproxy-connector
# connect`, installs a boot service that runs *as you* (User=<you>), so the connector —
# and the Claude Code it drives — runs under your account.
#
# Do NOT run the connector as root. Claude Code refuses `--dangerously-skip-permissions`
# under root, so it hangs. `| sudo sh` is fine for the install itself (binary →
# /usr/local/bin, config stays in your home, and the service still runs as the invoking
# user) — but don't install from a root login shell, which would make the service run as
# root.
#
...
# Override any of them with CCPROXY_CONNECTOR_BASE / CCPROXY_API_BASE / CCPROXY_WS_BASE.
set -eu

CONNECTOR_BASE="${CCPROXY_CONNECTOR_BASE:-__CONNECTOR_BASE__}"
API_BASE="${CCPROXY_API_BASE:-__API_BASE__}"
WS_ORIGIN="${CCPROXY_WS_BASE:-__WS_BASE__}"

BIN_NAME="ccproxy-connector"
...
printf "\n${B}ccproxy connector${Z} ${DIM}· installer${Z}\n\n"

# --- privilege + install locations ------------------------------------------
# Config + the private tmux always live in the *invoking* user's home and are owned by
# them, because `ccproxy-connector connect` installs a system service that runs as that
...
if command -v curl >/dev/null 2>&1; then
  ...
  die "need curl or wget to download the $BIN_NAME binary"
fi
...
case "$os" in
  Linux)  os=linux ;;
  Darwin) os=darwin ;;
  *) die "unsupported OS: $os (the connector runs on Linux and macOS)" ;;
esac
case "$arch" in
  x86_64|amd64)  arch=amd64 ;;
  aarch64|arm64) arch=arm64 ;;
  *) die "unsupported arch: $arch (the connector ships amd64 and arm64)" ;;
esac
target="$os-$arch"
step "Platform"
ok "$target"
...
step "Installing $BIN_NAME"
mkdir -p "$BIN_DIR"
tmp="$(mktemp)"
dl "$CONNECTOR_BASE/connector/latest/$target/$BIN_NAME" "$tmp" \
  || die "could not download $CONNECTOR_BASE/connector/latest/$target/$BIN_NAME"
install -m 0755 "$tmp" "$BIN_DIR/$BIN_NAME"; rm -f "$tmp"
ok "$BIN_DIR/$BIN_NAME"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
  *)
...
if command -v tmux >/dev/null 2>&1; then
  ...
  if dl "$CONNECTOR_BASE/connector/latest/$target/tmux" "$tmp" 2>/dev/null && [ -s "$tmp" ]; then
    ...
    warn "the login screen won't start until a tmux is on PATH or at $CFG_DIR/bin/tmux."
  fi
  rm -f "$tmp"
fi
...
#   - "base" — enrolment and the request/response API. Always the API base
#     (<origin>/ccproxy).
#   - "ws"   — the persistent control channel the resident connector dials out on. Empty
#     (the default) leaves that derivation to the connector, which turns "base" into a
#     ws(s):// … /machine/link URL itself. A deployment whose edge strips the WebSocket
#     Upgrade header can bake __WS_BASE__ (a plain http(s) origin) to point just this
#     channel at a WS-capable endpoint, leaving enrolment/API/downloads untouched.
# CCPROXY_WS_BASE overrides it manually (also a plain origin).
ws=""
if [ -n "$WS_ORIGIN" ]; then
  case "$WS_ORIGIN" in
...
# to root for a system-wide service if needed.
printf "\n${G}Installed.${Z} One more command to go online:\n\n"
printf "    ${B}$BIN_NAME connect --token <device-token>${Z}\n\n"
printf "${DIM}The device token is shown in the ccproxy console when the machine is created.\n"
printf "Manage later:  $BIN_NAME status · $BIN_NAME disconnect · $BIN_NAME uninstall${Z}\n\n"
