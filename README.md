...
- `.env` — `SUPERADMIN_PASSWORD`, `ENGINE_SECRET`, and the published `NGINX_HTTP_PORT` (default 80).
...
Provisioning drives the machine over SSH, so a machine must be, before `POST /machine`:

- **SSH-reachable** from the backend, as `root` (default) or a `sudo`-capable user
  (`sshUser`/`sshPort` are set on the machine).
- Authorizing the **operator public key** (`GET /provisioning/ssh-pubkey`) in that user's
  `authorized_keys`.
- Have **Claude Code** on `PATH` (`claude`), **Node.js**, **tmux**, and **openssl**/`ca-certificates`.
